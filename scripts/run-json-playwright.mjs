...
function e2eEnv(storeFile, overrides = {}) {
  return {
    ...process.env,
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "vota-wtf-e2e-"));
const storeFile = path.join(tempDir, "store.json");
const env = e2eEnv(storeFile);
const playwright = playwrightBin();

if (!fs.existsSync(playwright)) {
  console.error("Playwright is not installed. Run npm install, then rerun npm run e2e:json.");
  process.exit(1);
}

console.log(`Using isolated JSON store: ${storeFile}`);
run("node", ["-r", "./tests/register-ts.cjs", "scripts/seed-e2e.ts"], { env });
run(playwright, ["install", "chromium"], { env });
run(playwright, ["test"], { env });
