...
alter table ledger_entries
  ...
  check (type in ('starter_credit', 'prediction_commit', 'test_checkout_credit', 'void_refund', 'resolution_credit'));

create unique index if not exists one_resolution_credit_per_participant_market
  on ledger_entries (participant_id, market_id)
  where type = 'resolution_credit' and market_id is not null;
...
begin
  ...
  values (
    ...
    'market',
    p_market_id::text,
    jsonb_build_object('outcomeId', p_outcome_id, 'note', v_note, 'settledCount', v_settled_count, 'settledCredits', v_settled_credits),
    p_ip
  );
