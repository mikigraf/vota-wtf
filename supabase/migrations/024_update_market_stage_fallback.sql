...
begin
  ...
  set title = trim(p_title),
      ...
      show_on_stage = case when v_market.status = 'voided' then false else coalesce(p_show_on_stage, show_on_stage) end,
  ...
  if not v_market.show_on_stage then
    ...
    where id = v_market.event_id
      and featured_market_id = v_market.id;
  end if;
