...
export default async function AuditPage({
  ...
  searchParams: Promise<{ action?: string | string[]; entityType?: string | string[]; q?: string | string[] }>;
}) {
  ...
  const actions = uniqueSorted(store.adminAuditLogs.map((log) => log.action));
  const entityTypes = uniqueSorted(store.adminAuditLogs.map((log) => log.entityType));
          ...
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <p className="text-sm font-semibold text-white/55">
                Showing {logs.length} of {store.adminAuditLogs.length} recorded admin actions.
              </p>
            </div>
          </div>
