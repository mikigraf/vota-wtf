    ...
    return (
      <Shell className="bg-admin">
        <Container className="grid gap-6">
          <AdminNav />
          <Card>Event not found: {eventSlug}</Card>
        </Container>
      </Shell>
    );
  }
    ...
    <Shell className="bg-admin">
      <Container className="grid gap-6">
        ...
        <Card className="bg-paper">
          ...
          <h2 className="mt-1 text-xl font-black">{event?.name || DEFAULT_EVENT_SLUG}</h2>
        ...
        <section className="grid gap-3 md:grid-cols-4">
          ...
          <Stat label="Checkout interest" value={metrics.intentCount} />
          <Stat label="Intent EUR value" value={euro(metrics.intentProjectedEur)} />
        </section>
        <Card>
          <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
            ...
            <p className="text-sm font-bold text-faded">{metrics.intentClicks} total clicks, repeats hidden in the count.</p>
          </div>
          <div className="grid gap-2">
            {checkoutIntents.length === 0 ? (
              ...
              checkoutIntents.map((intent) => {
                ...
                return (
                  <div key={intent.id} className="grid gap-2 rounded-xl bg-paper p-3 md:grid-cols-[1fr_110px_100px_130px_140px] md:items-center">
                    <span>
                      <strong>{participant?.nickname || intent.participantId}</strong>
                      <span className="font-mono-vota block text-[10px] font-bold uppercase text-faded">{participant?.role || "unknown"}</span>
                    </span>
                    <span>{intent.clickCount} click{intent.clickCount === 1 ? "" : "s"}</span>
                    <span>{euro(intent.amountEur)}</span>
